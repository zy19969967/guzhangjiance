...
def get_files(root, dataset, faults, fault_label, signal_size, condition=3, stride=None):
    data, actual_labels = [], []
...
def data_transforms(normlize_type="-1-1", apply_normalize=True):
    train_transforms = [aug.Reshape()]
    val_transforms = [aug.Reshape()]
    if apply_normalize:
        train_transforms.append(aug.Normalize(normlize_type))
        val_transforms.append(aug.Normalize(normlize_type))
    train_transforms.append(aug.Retype())
    val_transforms.append(aug.Retype())
    transforms = {
        'train': aug.Compose(train_transforms),
        'val': aug.Compose(val_transforms)
    }
    return transforms
...
class dataset(object):
    ...
    def __init__(self, args, dataset, source_idx, condition=2, balance_data=False, test_size=0.2):
        ...
        self.data, self.actual_labels = get_files(data_root, dataset, faults, fault_label, signal_size, condition=condition, stride=stride)
        self.transform = data_transforms(normlize_type, apply_normalize=(dataset != "ROBOT"))
    ...
    def data_preprare(self, is_src=False):
        ...
        if is_src:
            ...
            if self.args.target_eval_mode == 'full':
                train_dataset = data_utils.dataset(list_data=data_pd, transform=self.transform['train'])
                val_dataset = data_utils.dataset(list_data=data_pd, transform=self.transform['val'])
